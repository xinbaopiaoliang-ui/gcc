...
### 阶段 85：v0.7.10 面板节点页后台化改版

- [x] 节点首页标题调整为“服务器管理”，增加页面说明，风格向传统后台管理页收敛。
- [x] 顶部统计改为 6 个轻量服务器状态卡，展示服务器总数、在线、离线/失联、待更新、待同步和 TCP/UDP。
- [x] 节点列表改为服务器管理表格布局，节点名称、节点 ID、状态、入口协议、版本策略、最后上报和操作分列展示。
- [x] 行内展示服务器图标、在线状态点、内部 ID、最后心跳和区域，不新增后端不存在的 CPU/MEM/DISK 假数据。
- [x] 工具条改为“添加服务器 + 搜索 + 状态/区域筛选 + 当前统计标签”的后台管理样式。
- [x] 本地验证通过：`web/panel npm run build`。

### 阶段 86：v0.7.11 面板整体后台布局改版

- [x] 左侧导航从纯图标窄栏改为后台管理常见的宽侧栏，增加品牌区和菜单文字。
- [x] 顶部区域调整为白色管理栏，统一页面标题、说明文案和账号操作区。
- [x] 内容区改为统一的 `24px` 后台页边距，节点页、流量页、会话页、策略页、账号页和系统页保持一致。
- [x] 移动端把侧栏降级为横向图标导航，避免宽菜单挤压内容。
- [x] 本阶段只调整控制面板视觉布局，不修改节点、业务后台和客户端协议。
- [x] 本地验证通过：`web/panel npm run build`。

### 阶段 87：v0.7.12 面板菜单刷新恢复

- [x] 面板菜单状态支持从 URL hash 恢复，例如 `#traffic`、`#sessions`、`#policies`。
- [x] 菜单切换时同步写入 `localStorage`，没有 hash 时也能按上次菜单恢复。
- [x] 监听浏览器 hash 变化，支持手动修改地址或浏览器前进/后退后切换菜单。
- [x] 保留账号权限边界，非管理员账号刷新到管理菜单时仍会回落到服务器管理。
- [x] 本阶段只调整前端导航状态，不修改控制面板后端、节点协议和业务后台接口。

### 阶段 88：v0.7.13 节点资源负载上报与展示

- [x] 节点上报 payload 增加 `system` 快照，包含 CPU、内存、磁盘、网络速率和 load average。
- [x] Linux 节点从 `/proc/stat`、`/proc/meminfo`、`/proc/net/dev` 和 `statfs("/")` 采集资源负载；非 Linux 构建返回空快照，保证本地 Windows 开发可编译。
- [x] 控制面板节点列表和节点详情从最新 `panel_node_reports.raw_json.system` 读取 `latest_system`，不新增数据库字段和 SQL 迁移。
- [x] 节点列表新增“负载”列，展示 CPU、内存、磁盘进度条；节点详情新增完整负载卡，展示容量和网络上下行速率。
- [x] 本阶段需要升级节点后才会有新负载数据；旧节点未上报时前端显示“等待上报”。
