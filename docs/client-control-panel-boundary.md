...
节点
  ...
  2. 校验 token 权限、flow metadata、本地 route_policies
  3. 转发 TCP/UDP
...
节点配置了 `route_policies` 后，`OPEN_TCP` 和 `OPEN_UDP` 都必须带完整 metadata。

| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `game_id` | 是 | 必须在 token `game_ids` 里 |
| `policy_id` | 是 | 必须在 token `policy_ids` 里，并且节点本地策略存在 |
| `rule_id` | 是 | 必须属于该 `policy_id` |
...
| `target_denied` | 命中规则、policy_id、rule_id、目标地址或端口不匹配 |
...
| `target_denied` | 业务配置里的 `game_id/policy_id/rule_id/target/port` 是否和节点策略一致 |
