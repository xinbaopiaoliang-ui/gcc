...
- 当前默认模式下，节点放行依据是：token 授权 + OPEN metadata + 节点基础安全边界。游戏规则命中由客户端根据业务后台配置完成。
...
| `policy_id` | 是 | 后台策略 ID。 |
| `rule_id` | 建议 | 命中的具体规则 ID，用于日志、统计和排查；当前默认模式不靠它放行。 |
...
节点当前默认使用 `route_policies.mode: "client_decision"`。该模式下会校验：
...
target_host + target_port 是否触发节点基础安全拦截
```

也就是说，客户端必须自己根据业务后台下发的规则决定是否加速某个目标。节点不再按本地策略逐条匹配 `target_host`、`target_port` 和 `rule_id`。
...
当 token 配置了 `game_ids`、`policy_ids` 和 `config_revision` 时，这些字段会参与 `OPEN_TCP` / `OPEN_UDP` 强校验。客户端必须保证 OPEN metadata 和 token 授权一致。
...
- token claims 校验 `game_ids` / `policy_ids` / `config_revision`。
- `route_policies.mode: "client_decision"`，节点按客户端决策转发，不再逐条匹配业务规则。
- `/sessions` 输出 `game_id` / `policy_id` / `rule_id`。
- metrics 按游戏和策略聚合。

当 token 没有限制 `game_ids`、`policy_ids`、`config_revision` 时，为兼容旧联调，节点允许不带 metadata 的测试流量。正式客户端 token 必须带这些授权字段，因此客户端也必须按本文发送 metadata，否则 `OPEN_TCP` / `OPEN_UDP` 会返回 `target_denied`。
